...
resource "null_resource" "integration_test_run" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      var.pre_test_setup,
      "echo excluded_tests is: ${var.excluded_tests}",
      "go test ${var.test_dir} -p 1 -timeout 1h -computeType=EC2 -bucket=${var.s3_bucket} -plugins='${var.plugin_tests}' -excludedTests=none -cwaCommitSha=${var.cwa_github_sha} -caCertPath=${var.ca_cert_path} -proxyUrl=${module.linux_common.proxy_instance_proxy_ip} -instanceId=${module.linux_common.cwagent_id} -v",
    ]
  }
